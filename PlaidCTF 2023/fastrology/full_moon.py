...
def compute(rl, rr):
    ...
    for warmup_len in range(rl, rr):
        xorShift = xorshift()
        equations = [xorShift.getRandom() for _ in range(320)]

        prefixs = [-1] * warmup_len
        for i in range(125):
            prefixs.append(prefix[i])
            prefixs.append(-1)
        prefixs += [-1] * (320 - len(prefixs))
        outputs = []
        for i in range(0, 320, 64):
            outputs += prefixs[i:i+64][::-1]
        assert len(outputs) == 320 
        solver = Solver()
        out = [0b000, 0b00, 0b0, 0b01, -1, 0b10, 0b1, 0b11, 0b111]
        bit = [3, 2, 1, 2, 0, 2, 1, 2, 3]
        ...
        for i in range(prefix_len+128):
            ...
            rand_max = math.floor(rand.getRandom() * 4)
            distortion_len = math.floor(i/125)
            for j in range(distortion_len):
                index ^= math.floor(rand.getRandom() * rand_max)
            index = min(index, len(alphabet) - 1)

...
flag = 'zer0pts{this_is_fake_flag}'.

keys = [md5(os.urandom(3)).digest() for _ in range(3)]
key_long = [bytes_to_long(keys[i]) for i in range(3)]
...
while True:
    ...
    if choice == 1:
        ...
        ciphertext = b":".join([hexlify(x)
                                for x in [iv1, iv2, ciphertext]]).decode()
        print("here's the ciphertext: {}".format(ciphertext))

    elif choice == 2:
        ciphertext = input("your ciphertext: ")
        iv1, iv2, ciphertext = [unhexlify(x)
                                for x in ciphertext.strip().split(":")]
        plaintext = decrypt(ciphertext, iv1, iv2)
        print("here's the plaintext(hex): {}".format(
            hexlify(plaintext).decode()))
